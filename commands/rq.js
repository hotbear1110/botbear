	...
	execute: async (channel, user, input, perm) => {
		try {
			...
			if (input[2]) {
				...
				uid = await got(`https://api.ivr.fi/v2/twitch/user?login=${input[2]}`).json()[0];
				uid = uid.id;
				realuser = input[2];
			}
			...
			while (!messageFound) {
				...
				rl = await got(`https://logs.ivr.fi/channel/${realchannel}/userid/${uid}/${year}/${month}?json`).json();

				realmessages = rl.messages.filter((message) => {
                    if (message.type !== 1) {
                        return false;
                    }
                    return true;
